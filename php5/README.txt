...
$prefix - '/dev/API/' - Your individual, private API prefix
$system - 'test' - Your system ID. Set to 'prod' when done testing

Let's give it a spin. Fire up your php5 CLI. On *nix and Linux it's

    % time php5 ./billin-test.php5

The output for the command execution should be something like:
...
--------------------------------------------------------------------------------
    ...
    new BillinSession()
        ...
        To login the variables set in config.php5 are used: $user, $password,
        $api_key. The preferred way to login is to use $api_key instead of the
        $password
    ...
    function BillinSession->call_api($fn, $args = array(), $named_args = array(), 
                                     $post_args = array(), $return_json = True) 

        Invokes any API call named $fn. The mandatory arguments for invocation
        are $args, named arguments are $named_args and $post_args are used to
        invoke POST instead of a GET HTTP method for the API call.

        The $return_json argument defaulting to True causes API call result to
        be formatted as JSON

        Returns API call result formatted as JSON or HTML if $return_json is
        False. Note that you don't have to use this function if the scope of
        current SDK version is sufficient.
    ...
    function BillinSession->elt($n, $list = Null) 
        ...
        Examples: elt(0), elt(5), elt(1, ref(-2))
                  $sess->list_customer_subscriptions()
                  $sess->elt(0) - get the first subscription in the system

    function ref($n) 

        Returns referal to returned value number $n in the transaction call history

        Examples: ref(-1), ref(0), ref(1)
    ...
    function amount($string) 
        ...
        Example: amount('100'), amount('100.22'), amount('100.23545')
             
    function keyword($val) 

        Returns keyword for use as a name in API calls.
        
        Example: keyword(payment), keyword(all)
	...
	function BillinSession->create_billing_data($billing_data_args) 
        ...
        in: http://billin.pl/upload/billin_1.0/doc/create.html

        Example $billing_data =
	...
	function BillinSession->create_customer($billing_data = Null, $customer_args = array())
        ...
        specified in: http://billin.pl/upload/billin_1.0/doc/create.html

        Example: $customer = $sess->create_customer($billing_data);
    ...
    function BillinSession->get_product_params($id, $customer = Null) 
        ...
        For a product with id = $id and customer = $customer, returns an object
        for product configuration using the configure_product method

        Example:   $product = $sess->get_product_params('CRM Complete - month', $customer)

    function BillinSession->configure_product($product, $changes) 
        
        Configure product for assignment. $product is returned by
        get_product_params invocation. $changes is an array mapping product
        definition tree parameter modifications to product configuration. Tree
        nodes are separated with the ! character.
    ...
    function BillinSession->create_subscription($customer = Null, $subscription_args = array()) 
        ...
        Creates and returns a subscription object. $subscription_args are described in:
        http://billin.pl/upload/billin_1.0/doc/create.html

        Example: $subscription = $sess->create_subscription($customer);

    function BillinSession->assign_product($product, $subscription = Null, $product_args = array()) 

        Assigns a configured product to a previously created $subscription. 
        Arguments for product creation are described in:
        http://billin.pl/upload/billin_1.0/doc/assign_product.html

        Example: $sess->assign_product($product, $subscription);

	function BillinSession->swap_product($product, $subscription = Null, $swap_args = array())

        Replaces a product in subscription with another. $swap_args are described in:
        http://billin.pl/upload/billin_1.0/doc/swap_product.html
    ...
    function BillinSession->list_customer_invoices($customer, $named_args = array())
        ...
        list and are described in:
        
        http://billin.pl/upload/billin_1.0/doc/list_data.html

        Note - symbols :initial, :normal and so on indicate keywords. The are
        created in PHP with keyword('initial') and similar invocations of the
        keyword function.
    ...
    function get_payu_pending_payment($customer = Null) 
        ...
        Returns pending payment information for PayU Payment provider. The data
        returned allow you to populate an HTML transaction form to invoke online
        payment with PayU. The important fields of returned object are id and
        amount. 
    ...
    function BillinSession->get_document_image($path, $invoice = Null, 
                                    $output_format = Null, $image_type = Null)

        Stores an invoice image in HTML or PDF in file $path. The $path must be
        writable. $invoice is an invoice object, $output_format is one of
        keyword(html), keyword(pdf). $image_type is one of keyword(original),
        keyword(copy).

        Example:   $sess->list_customer_invoices()
                   $sess->elt(0)
